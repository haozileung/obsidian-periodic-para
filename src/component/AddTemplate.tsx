...
export const AddTemplate = () => {
	...
	const createFile = async (values: any) => {
		...
		if (type === PARA) {
			...
			templateFile = `Templates/PARA/${paraActiveTab}s.md`;
			...
			templateFile = `Templates/PeriodicNotes/${periodicActiveTab}.md`;
		}
	...
	return (
		...
		>
			...
			>
				...
				>
					<Radio.Button value={PERIODIC_NOTES}>
						{PERIODIC_NOTES}
					</Radio.Button>
				...
				{type === PERIODIC_NOTES ? (
					...
					<>
						<Tabs
							...
							items={[PROJECT, AREA, RESOURCE, ARCHIVE].map(
								(item) => {
									return {
										label: item,
										key: item,
